...
export default function Account() {
  ...
  const { data: myStanding } = useQuery<{ tierKey: string; tierLabel: string; hasNextTier: boolean; progressPercent: number }>({
    queryKey: ["/api/standing/mine"],
    enabled: user?.role === "partner" || user?.role === "student",
  });
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col gap-6">
      ...
      {(user?.role === "partner" || user?.role === "student") && myStanding && (
        <Card>
