...
test.describe("Login page — branding + hero photo", () => {
  for (const vp of [
    ...
    test(`renders on ${vp.name} with Partner Platform branding, no "Institute" tagline`, async ({ page }) => {
      await page.setViewportSize({ width: vp.width, height: vp.height });
      await page.goto("/");
      await expect(page.getByTestId("button-submit-login")).toBeVisible();
      await expect(page.getByText("Partner Platform — sign in to your account")).toBeVisible();
